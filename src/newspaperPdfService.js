...
exports.generateNewspaperPDF = generateNewspaperPDF;
const jspdf_1 = __importDefault(require("jspdf"));
async function generateNewspaperPDF(articles, settings) {
    try {
...
function addNewspaperHeader(pdf, pageWidth) {
    // Main masthead
...
async function layoutNewspaperArticles(pdf, rankedArticles, margin, contentWidth, startY, pageHeight, targetLanguage) {
    ...
    for (let index = 0; index < rankedArticles.length; index++) {
        ...
        if (isTopStory) {
            ...
            const articleHeight = await addRegularArticle(pdf, ranked, xPosition, column === 0 ? yPosition : yPosition, columnWidth, targetLanguage);
            if (column === 1) {
                yPosition += articleHeight;
            }
        }
    }
    return yPosition;
}
async function addTopStory(pdf, ranked, margin, contentWidth, yPosition, targetLanguage, pageHeight) {
    ...
    // Summary (3 paragraphs, NYT-style)
...
async function addRegularArticle(pdf, ranked, xPosition, yPosition, columnWidth, targetLanguage) {
    const article = ranked.article;
    let localY = yPosition;
    // Title (medium, bold, NYT-style)
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(13);
    pdf.setTextColor(0, 0, 0);
    ...
    // Summary (3 paragraphs, condensed for column)
    pdf.setFont('helvetica', 'normal');
    const fontSize = 9;
    pdf.setFontSize(fontSize);
...
async function rankArticlesByImportance(articles, settings) {
    ...
    try {
        ...
        const articlesInfo = articles.map((a, i) => `${i + 1}. ${a.title}\n   ${a.contentSnippet || a.content?.slice(0, 150) || ''}`).join('\n\n');
...
async function extractArticleImages(rankedArticles) {
...
function extractMainImageFromHTML(html, baseUrl) {
